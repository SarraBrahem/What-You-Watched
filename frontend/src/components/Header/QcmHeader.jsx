    ...
    <header className="flex md:hidden lg:hidden bg-black text-white p-4 justify-between items-center border-b border-blue-900 relative">
      ...
      <div className={`absolute top-0 left-0 w-4/5 h-screen bg-black bg-opacity-95 z-10 transform ${isMenuOpen ? "translate-x-0" : "-translate-x-full"} transition-transform duration-300 ease-in-out`}>
          <div className="flex flex-col items-center">
            ...
            <img src={`${process.env.PUBLIC_URL}/images/WYW.png`} alt="Logo" className="h-12 mt-5" />
            ...
            <li><Link to="/qcm" className="text-blue-500 hover:text-blue-600 font-bold text-2xl">QCM</Link></li>
            </ul>
            <ul className="mt-48 space-y-6 text-center">
              <li><Link to="/UserProfile" className="hover:text-blue-200 font-semibold">Profil</Link></li>
              <li><Link to="/watchlist" className="hover:text-blue-300 font-semibold">Ma Watchlist</Link></li>
              {isAuthenticated ? (
              // Affichez l'avatar si l'utilisateur est connecté
              <li> <Link to="/UserProfile" className="hover:text-indigo-300">
