...
function Header() {
  ...
  return (
    <><header className="hidden md:flex bg-black text-white p-4 justify-between items-center border-b border-gray-900 relative">
    ...
    <header className="flex md:hidden lg:hidden bg-black text-white p-4 justify-between items-center border-b border-gray-900 relative">
      ...
      <div className={`absolute top-0 left-0 w-4/5 h-screen bg-black bg-opacity-95 z-10 transform ${isMenuOpen ? "translate-x-0" : "-translate-x-full"} transition-transform duration-300 ease-in-out`}>
        <div className="flex flex-col items-center">
          ...
          <ul className="mt-48 space-y-6 text-center">
            <li><Link to="/UserProfile" className="hover:text-indigo-300 font-semibold">Profil</Link></li>
            <li><Link to="/watchlist" className="hover:text-indigo-500 font-semibold">Ma Watchlist</Link></li>
            {isAuthenticated ? (
            // Affichez l'avatar si l'utilisateur est connecté
            <li>
              <Link to="/UserProfile" className="hover:text-indigo-300">
      ...
      <Link to="/" className="flex items-center">
        <img src={`${process.env.PUBLIC_URL}/images/RondSansFond.png`} alt="Logo" className="h-20 shadow-sm rounded-full shadow-white" />
      </Link>
    </header></>
  );
}
